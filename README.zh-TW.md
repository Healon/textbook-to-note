...
- **Template 驅動抓取** — 每種主題有固定模板。這是關鍵：它告訴 AI 該去書裡找什麼，也讓每份筆記形狀一致，讓**你**讀得更快。像開頭的 Summary、Management algorithm 這種段落，存在本身就是為了幫助理解，不只是放資料
...
筆記流程有選用的 hook 點，可從外部來源補強草稿 — 臨床實證 API、給付／規範資料庫、文獻搜尋。為了控制專案規模，這些不放進本 repo；流程文件標明它們接入的位置，讓你接上自己領域的來源。
