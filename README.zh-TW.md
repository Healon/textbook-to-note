...
- **書層級表格可靠度旗標** — 有些書天生對表格抽取充滿敵意：當一本書很大比例的表格觸發結構 QC flag 時，個別的 `⚠️` 標記無法傳達出「*整本書*的表格都不可信」。QC gate 看得到結構、看不到「值綁到錯的列」的錯綁，因此高 flag 率正是「這裡任何一張表沒對過 PDF 都別信」的代理指標。若一本書 ≥40% 的表格（且至少 10 張）被 QC flag，markdown 最上方會掛一個 `> [!caution]` banner，告訴下游 LLM 每張表都要對照原始 PDF 查核，並把 `reliability_flagged` / `flag_rate` 寫進 per-book 統計。純偵測。（某本密集藥理參考書實測 66%。）
